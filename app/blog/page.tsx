...
export default async function BlogIndex() {
    try {
        console.log("Starting Blog Page Render..."); // Server-Side Log
        ...
        try {
            // Check if ENV vars exist before even calling the client to prevent crash
            if (process.env.NEXT_PUBLIC_SUPABASE_URL && process.env.NEXT_PUBLIC_SUPABASE_ANON_KEY) {
                ...
                console.warn("Supabase env vars missing, skipping DB fetch.");
            }
        } catch (error) {
            console.warn("Supabase fetch failed, continuing with static blogs.", error);
        }

        // Combine with dynamically generated clinical guides (100+ pages)
        let autoBlogs: any[] = [];
        try {
            autoBlogs = NeoBlogEngine.getAllAutoBlogs();
        } catch (e) {
            console.error("NeoBlogEngine Failed:", e);
        }
                                        ...
                                        <div className="relative h-48 w-full bg-slate-200 dark:bg-white/5">
                                            {post.image ? (
                                                ...
                                                <div className="w-full h-full flex flex-col items-center justify-center text-slate-400 p-6 text-center">
                                                    ...
                                                    <span className="text-xs font-bold uppercase tracking-widest text-slate-600 dark:text-slate-400">{post.category}</span>
                                                </div>
                                            )}
                                        </div>
        ...
        console.error("CRITICAL BLOG PAGE CRASH:", CRITICAL_ERROR);
        return (
            <div className="min-h-screen pt-40 px-6 text-center">
                <h1 className="text-3xl font-bold mb-4">Blog Section Unavailable</h1>
                <p className="text-slate-500 mb-8">We are currently upgrading our clinical intelligence database.</p>
                <div className="p-4 bg-red-50 text-red-600 rounded-lg inline-block text-left text-sm font-mono max-w-2xl overflow-auto">
                    {JSON.stringify(CRITICAL_ERROR, Object.getOwnPropertyNames(CRITICAL_ERROR))}
                </div>
            </div>
        )
    }
