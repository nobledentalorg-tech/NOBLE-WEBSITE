...
const Hero: React.FC<HeroProps> = ({ onBookClick }) => {
  ...
  useEffect(() => {
    ...
    function draw() {
      ...
      particles.forEach((p) => {
        p.angle += 0.0015;
        p.x += p.speedX + Math.cos(p.angle) * 0.4;
        p.y += p.speedY + Math.sin(p.angle) * 0.4;
        ...
        // Optimized: Single simple fill instead of complex radial gradient per frame
        context.fillStyle = 'rgba(84, 179, 255, 0.15)';
        context.beginPath();
        context.arc(p.x, p.y, p.orbit, 0, Math.PI * 2);
        context.fill();

        context.fillStyle = 'rgba(255, 158, 94, 0.1)';
        context.beginPath();
